...
const navItems: NavItem[] = [
  ...
  { label: "Create Escrow", href: "/dashboard/escrow-create", icon: PlusSquare },
  ...
  return (
    ...
    >
      <div className={cn("relative flex items-center", isCollapsed ? "justify-center" : "justify-between")}>
        <div className={cn("flex items-center", isCollapsed ? "justify-center" : "gap-3")}>
          ...
          <div className={cn("transition-opacity", isCollapsed && "lg:hidden")}>
            <p className="text-sm font-semibold text-slate-900 dark:text-white">SplitRent</p>
            <p className="text-xs text-slate-600 dark:text-slate-300">Dashboard</p>
          </div>
        </div>
      ...
      <button
        ...
        aria-label={walletAddress ? "Copy wallet address" : "Wallet not connected"}
        className={cn(
      ...
      >
        <p className="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">Wallet</p>
        <div className="mt-1 flex items-center justify-between gap-2">
          <p className="text-base font-semibold text-slate-900 dark:text-white">{shortAddress ?? "Not connected"}</p>
          {walletAddress ? (
        ...
        <p className="mt-1 text-xs text-slate-600 dark:text-slate-300">
          {walletOnTestnet ? "Testnet Connected" : walletOnMainnet ? "Mainnet Connected" : "No network"}
        </p>
      </button>

      <nav className={cn("mt-5 grid gap-2", isCollapsed && "lg:justify-items-center")}>
        {navItems.map((item) => {
      ...
      <div className={cn("mt-auto space-y-3 pt-4", isCollapsed && "lg:hidden")}>
        <div className="rounded-2xl border border-white/70 bg-white/70 p-3 text-xs text-slate-700 dark:border-white/10 dark:bg-slate-900/60 dark:text-slate-300">
          <p className="font-semibold text-slate-900 dark:text-white">Balance sync</p>
          <p className="mt-1">Auto-refresh every 30s</p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <RefreshCw className={cn("mr-2 h-4 w-4", isRefreshingBalance && "animate-spin")} />
            Refresh
          </Button>
        </div>
      ...
      <div className={cn("mt-auto hidden flex-col items-center gap-2 pt-4 lg:flex", !isCollapsed && "lg:hidden")}>
        <ThemeToggle />
        <Button
          variant="outline"
          size="icon"
